 ...
 *   Kalle Alm, kalle@enrogue.com, Zealotry client
 *
...
const __zealotry_version   = "0.1";
const __zealotry_condition = "green";
const __zealotry_suffix    = "";
const __zealotry_guid      = "ff1b3636-869f-4ee5-9dae-d2e12b8751d2";
const __zealotry_locale    = "0.1";
...
function initStatic()
{
    ...
    var ver = __zealotry_version + (__zealotry_suffix ? "-" + __zealotry_suffix : "");
...
function initIcons()
{
    ...
    const iconName = "zealotry-window";
    ...
    if (client.host == "Firefox")
    {
        ...
        sourceDir.append("{" + __zealotry_guid + "}");
...
function processStartupURLs()
{
        ...
        0 in window.arguments && typeof window.arguments[0] == "string")
    {
        ...
        var urlMatches = url.match(/^zealotrys?:\/\/\/?(.*)$/)
        if (urlMatches)
        {
            if (urlMatches[1])
            {
    ...
    if (!wentSomewhere)
    {
        ...
        for (var i = 0; i < ary.length; ++i)
        {
            if (ary[i] && ary[i] == "zealotry:///")
            {
                ...
                // support any more; replace with the harmless zealotry:// URL.
                ary[i] = "zealotry://";
    ...
    if (client.viewsArray.length > 1 && !isStartupURL("zealotry://"))
    {
...
function insertLink (matchText, containerTag, data)
{
    ...
    anchor.setAttribute ("class", "zealotry-link");
...
function insertRheet (matchText, containerTag)
{
    ...
    anchor.setAttribute ("class", "zealotry-rheet zealotry-link");
    //anchor.setAttribute ("target", "_content");
    insertHyphenatedWord (matchText, anchor);
    containerTag.appendChild (anchor);
}
...
function insertSmiley(emoticon, containerTag)
{
    ...
    span.setAttribute ("class", "zealotry-emote-txt");
     ...
     * after it with a zealotry-emote:after css rule. using
     * zealotry-emote-txt:after is not good enough because it does not allow us
     * to turn off the emoticon text, but keep the image.  ie.
     * zealotry-emote-txt { display: none; } turns off
     * zealotry-emote-txt:after as well.*/
    span = document.createElementNS ("http://www.w3.org/1999/xhtml",
                                     "html:span");
    span.setAttribute ("class", "zealotry-emote");
    span.setAttribute ("type", type);
...
function showCtrlChar(c, containerTag)
{
    var span = document.createElementNS ("http://www.w3.org/1999/xhtml",
                                         "html:span");
    span.setAttribute ("class", "zealotry-control-char");
    if (c == "\t")
    {
...
function insertInlineButton(text, containerTag, data)
{
    ...
    link.setAttribute("href", "x-zealotry-command:" + encodeURI(command));
...
var testURLs =
    ["zealotry:", "zealotry://", "zealotry:///", "zealotry:///help", "zealotry:///help,needkey",
     "zealotry://zealotry.foo.org", "zealotry://foo:6666",
    "zealotry://foo", "zealotry://zealotry.foo.org/", "zealotry://foo:6666/", "zealotry://foo/",
    "zealotry://zealotry.foo.org/,needpass", "zealotry://foo/,isserver",
    "zealotry://moznet/,isserver", "zealotry://moznet/",
    "zealotry://foo/zealotry", "zealotry://foo/zealotry/",
    "zealotry://zealotry.foo.org/?msg=hello%20there",
    "zealotry://zealotry.foo.org/?msg=hello%20there&ignorethis",
    "zealotry://zealotry.foo.org/%23mozilla,needkey?msg=hello%20there&ignorethis",
    "invalids",
    "zealotry://zealotry.foo.org/,isnick"];
...
function parseIRCURL (url)
{
    ...
    rv.host = null;
    rv.target = "";
    rv.port = (rv.scheme == "zealotrys" ? 9999 : 6667);
    rv.msg = "";
    rv.pass = null;
    rv.key = null;
    rv.charset = null;
...
function tabdnd_drop (aEvent, aXferData, aDragSession)
{
    ...
    else if (url.search(/^zealotrys?:\/\//i) != -1)
    ...
    if (fromUser && msgtype.match(/^(PRIVMSG|ACTION|NOTICE)$/))
    {
        ...
        if (nickURL)
        {
            var nick_anchor =
                document.createElementNS("http://www.w3.org/1999/xhtml",
                                         "html:a");
            nick_anchor.setAttribute("class", "zealotry-link");
