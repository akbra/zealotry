...
const ZEALOUS_VERSION = "0.7.11.10";
...
function pollFrames() {
    ...
    /*    if (fiskmas == 0) {
	dump("polling frames! ");
	dump("ldoc: " + (ldoc == null ? "null" : ldoc.pageLoaded));
	dump(", cdoc: " + (cdoc == null ? "null" : cdoc.pageLoaded));
	dump(", rdoc: " + (rdoc == null ? "null" : rdoc.pageLoaded));
	dump("\n");
	}*/

    /*if (!fiskmas && cdoc != null && cdoc.pageLoaded != false) {
	var win = frames["center-frame"].window;
	dump("center frame's window.wrappedJSObject: " + win.wrappedJSObject + "\n") ;
	dump("center frame's window.wrappedJSObject.document: " + win.wrappedJSObject.document + "\n") ;
	dump("center frame's window.wrappedJSObject.document.pageLoaded: " + win.wrappedJSObject.document.pageLoaded + "\n") ;
	dump("center frame's document.wrappedJSObject: " + cdoc.wrappedJSObject + "\n") ;
	dump("center frame's document.wrappedJSObject.pageLoaded: " + cdoc.wrappedJSObject.pageLoaded + "\n") ;
	dump("dump of center frame's document.wrappedJSObject:\n");
	win = cdoc.wrappedJSObject;
	for (var e in win) {
	    try { dump(e + ": " + win[e] + "\n"); } catch (rr) { dump(e + " [exception thrown]\n"); }
	}
	dump("dump of cdoc:\n");
	for (var e in cdoc) {
	    try { dump(e + ": " + cdoc[e] + "\n"); } catch (rr) { dump(e + " [exception thrown]\n"); }
	...
	dump("============================================================\n");
	dump("dump of cdoc.documentElement:\n");
	for (var e in cdoc.documentElement) {
	    try { dump(e + ": " + cdoc[e] + "\n"); } catch (rr) { dump(e + " [exception thrown]\n"); }
	    }
	fiskmas = 1;
	}*/
	...
	ldoc.pageLoaded != false && cdoc.pageLoaded != false && rdoc.pageLoaded != false) { // above commented line does not work, because server cannot set vars; instead, our "false" becomes "undefined", hence !false
	...
	if (window
	    ...
	    && window.center_frame.document.wrappedJSObject 
	    && window.center_frame.document.wrappedJSObject.getElementById("output_body")) {
	    dump("hopping to mainStep()!\n");
	    mainStep();
	    return;
	}
    }
...
function mainStep()
{
    ...
    var output = window.center_frame.document.wrappedJSObject.getElementById("output_body");
    if (!output) {
    ...
    window.client.setClientOutput(realOutput); //output); // , scrollback.contentDocument.body);
    ...
    window.client.connect
        (window.center_frame.wrappedJSObject.getHost(),
         window.center_frame.wrappedJSObject.getPort(),
         onRead);
